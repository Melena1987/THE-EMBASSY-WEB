    ...
    <nav className={`fixed w-full z-[100] transition-all duration-500 ${navBackgroundStyles}`}>
      <div className="max-w-[1400px] mx-auto px-6 lg:px-6 xl:px-12 flex justify-between items-center relative z-[110]">
        ...
        <div className="hidden lg:flex items-center lg:space-x-3 xl:space-x-6">
          ...
          {/* AVANCE GLOBAL CUP PILL - Moved to the right of RESERVAR AHORA */}
          <button 
        ...
        <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="lg:hidden text-white p-2">
          <div className="w-6 h-5 flex flex-col justify-between">
            <span className={`w-full h-0.5 bg-white transition-all ${isMenuOpen ? 'rotate-45 translate-y-[9px]' : ''}`}></span>
            <span className={`w-full h-0.5 bg-white ${isMenuOpen ? 'opacity-0' : ''}`}></span>
            <span className={`w-full h-0.5 bg-white transition-all ${isMenuOpen ? '-rotate-45 -translate-y-[9px]' : ''}`}></span>
          </div>
        </button>
      </div>
      ...
      {/* Mobile Menu */}
      <div className={`lg:hidden fixed inset-0 bg-black z-[100] flex flex-col items-center justify-center transition-all duration-500 h-[100dvh] w-full ${isMenuOpen ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0'}`}>
        <div className="flex flex-col items-center space-y-8 px-6 text-center">
           {/* Mobile Next Event Banner */}
           <button 
             onClick={openAvanceGlobal}
             className="flex items-center gap-4 bg-white/5 px-6 py-4 rounded-3xl border border-gold/30 moving-gold-border mb-4"
           >
              <div className="w-12 h-12 rounded-full bg-black border border-white/10 flex items-center justify-center p-2">
                <img src={LOGOS.nextEvent} alt="Next Event" className="w-full h-full object-contain" />
              </div>
              <div className="text-left">
                <p className="text-[8px] font-black text-gold uppercase tracking-[0.3em] mb-1">{t('nav', 'nextEvent')}</p>
                <p className="text-sm font-black text-white uppercase italic tracking-tighter">AVANCE GLOBAL CUP</p>
              </div>
           </button>

           <a onClick={(e) => handleLinkClick(e, { path: '/eventos' })} className="text-2xl font-black uppercase tracking-widest">{t('nav', 'events')}</a>
           <a onClick={(e) => handleLinkClick(e, { path: '#instalaciones' })} className="text-2xl font-black uppercase tracking-widest">{t('nav', 'installations')}</a>
           <div className="flex gap-8 pt-4">
              <button onClick={() => setLanguage('es')} className={`text-sm font-black tracking-widest ${language === 'es' ? 'text-gold' : 'text-white/30'}`}>ES</button>
              <button onClick={() => setLanguage('en')} className={`text-sm font-black tracking-widest ${language === 'en' ? 'text-gold' : 'text-white/30'}`}>EN</button>
           </div>
           <a onClick={(e) => handleLinkClick(e, { path: '#contacto' })} className="bg-gold text-white px-10 py-5 rounded-full text-[10px] font-black uppercase tracking-[0.3em]">{t('nav', 'booking')}</a>
        </div>
      </div>
    </nav>
  );
