    ...
    <section ref={sectionRef} className="py-32 bg-black border-t border-white/5 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 text-center">
        <div className="mb-20 reveal">
          ...
          <h2 className="text-4xl md:text-7xl font-black uppercase italic tracking-tighter mb-8">COMPROMISO SOCIAL</h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-12 md:gap-24 items-center reveal">
          {SOCIAL_PROJECTS.map((proj, idx) => (
            ...
            >
              <div className="h-20 md:h-28 w-full overflow-hidden flex items-center justify-center opacity-40 group-hover:opacity-100 transition-all duration-700 transform group-hover:scale-110">
                <img 
                  src={proj.url} 
                  alt={proj.name} 
                  className="h-full w-auto object-contain grayscale group-hover:grayscale-0 transition-all duration-700" 
                />
              </div>
              <div className="h-[2px] w-8 bg-gold/30 group-hover:w-full transition-all duration-700"></div>
              <span className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.4em] text-white/30 group-hover:text-gold transition-colors text-center">
                {proj.name}
              </span>
            </a>
          ))}
