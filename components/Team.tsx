            ...
            {TEAM.map((member, idx) => (
              <div key={idx} className="reveal flex-none w-[85vw] md:w-auto snap-center group">
                <div className="relative overflow-hidden rounded-[2rem] mb-4 aspect-[4/5] bg-neutral-900 border border-white/5 group-hover:border-gold/30 transition-all duration-700">
                  ...
                  {/* Social overlay */}
                  <div className="absolute top-4 right-4 flex flex-col space-y-3 translate-x-12 group-hover:translate-x-0 transition-transform duration-500 delay-100">
                    {member.socials.instagram && (
                      <a 
                        href={member.socials.instagram} 
                        target="_blank" 
                        rel="noopener noreferrer" 
                        className="w-10 h-10 bg-black/60 backdrop-blur-xl border border-white/10 rounded-full flex items-center justify-center hover:bg-gold hover:text-white transition-all duration-300"
                        title={`${member.name} en Instagram`}
                      >
                    ...
                    {member.socials.twitter && (
                      <a 
                        ...
                        rel="noopener noreferrer" 
                        className="w-10 h-10 bg-black/60 backdrop-blur-xl border border-white/10 rounded-full flex items-center justify-center hover:bg-gold hover:text-white transition-all duration-300"
                        title={`${member.name} en X (Twitter)`}
                      >
